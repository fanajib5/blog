---
title: "About"
description: "Faiq Najib, Developer, Builder, Educator. Backend engineer specializing in Go & PostgreSQL, based in Jombang, East Java."
date: 2018-03-20T20:16:07+08:00
lastmod: 2026-04-15T00:00:00+07:00
draft: false
comments: false
---

I'm **Faiq Najib**, a **Developer, Builder, and Educator** based in Jombang, East Java. I build scalable backend systems, develop digital products, and share knowledge through teaching.

## Developer

Backend engineer with a primary focus on **Go (Golang)** and **PostgreSQL**. Experienced in building high-traffic systems, from a GPS fleet tracking platform with 370+ API endpoints to a SaaS platform for European clients.

- **Languages**: Go, PHP, SQL
- **Databases**: PostgreSQL, TimescaleDB, MySQL, Redis
- **Infrastructure**: Docker, Linux, Nginx, CI/CD, Apache Kafka
- **Specializations**: REST API & microservices, legacy migration (PHP → Go), database optimization

See my work on the [Projects](/en/projects/) page.

## Builder

I don't just write code, I **build products**. PT Akordium Digital Berkah is my sole proprietorship focused on digital solutions for real businesses. Its first product, MIS-APAR, digitized an entire fire extinguisher business operation that was previously manual.

I also have experience delivering freelance projects across various sectors, from construction (inventory management) to banking (process automation & Open API portal).

Need a solution for your business? Check out the [Services](/en/services/) page.

## Educator

Online tutor at **Universitas Terbuka** for the Information Systems program, with a strong academic background:

- **Ir. (Computer Engineering)**: Institut Teknologi Indonesia (CGPA: 3.90/4.00)
- **M.Com. (Information Systems)**: Institut Teknologi Sepuluh Nopember (CGPA: 3.81/4.00, Best Master Thesis)
- **B.Eng. (Telecommunications Engineering)**: Telkom University (CGPA: 3.49/4.00)
- **A.A.S. (Telecommunications Engineering)**: Politeknik Elektronika Negeri Surabaya (CGPA: 3.42/4.00)

My teaching focus: ensuring every participant **understands fundamental concepts**, not just follows tutorials, because a strong foundation enables adaptation to any technology in the future.

Full details on the [Teaching](/en/teaching/) page.

## Collaboration

Interested in working together? Check out the [Services](/en/services/) page or [get in touch](/en/contact/) directly.

## Find me on...

* Telegram: [@fanajib5](http://t.me/fanajib5)
* Twitter: [@fanajib5](https://twitter.com/fanajib5)
* GitHub: [fanajib5](https://github.com/fanajib5)
* LinkedIn: [Faiq Najib](https://linkedin.com/in/fanajib5)
* Email: [faiq.najib@gmail.com](mailto:faiq.najib@gmail.com)
* CV: [faiq-resume.pdf](/cv/faiq-resume.pdf)

## Trivia

* [/uses](/uses)
* [/komik](/komik)

<br/></br>

<img style="max-width: 30%; float=left;" src="Faiqnajib-handwriting-_1_.webp" alt="Faiq Najib's signature" title="Faiq Najib Al-Aziz">

<br/></br>

---

This site is built with [Hugo](https://gohugo.io/), open-sourced on [GitHub](https://github.com/fanajib5/blog) along with the Markdown source files for articles. You may use this site's source code under the [Mozilla Public License 2.0](https://opensource.org/license/mpl-2-0/). Articles, images, and other content follow the [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/deed.zh) license.
